...
store_keys = list(get_page_store_keys(
    ...
    default_keys=list(config["stores"].keys()),  # matches today's behavior until an admin edits the list
    require_connected=False,  # matches today's behavior: shows every store key in config, connected or not
))
store_names = {
